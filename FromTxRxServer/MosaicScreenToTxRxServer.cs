...
namespace FromTxRxServer
{
    ...
    public class TxRxServer
    {
        ...
        public static Utils_UdpMulti KvmUdpMulti = null;
        ...
        public static void LoadSynchroFromTxRxServer(string ip,int port)
        {
            if (KvmUdpMulti==null)
            {
                KvmUdpMulti = new Utils_UdpMulti();
                KvmUdpMulti.FuncUdpMultiMsg += MulticastServerRcvMsgAsync;
                KvmUdpMulti.InitialJoinGroup(hsConfig.MultiIp_SynchroTxServer,hsConfig.MultiPort_SynchroTxServer);
            }
        ...
        public static string MulticastServerRcvMsgAsync(string msg) //服务器的组播同步消息:udp 234.234.1.3 32008  同步状态/husion,所有设备
        {
            string rcv = null;
            ...
            else if (msges[0] == "online")
            {
                ...
                if (msges.Length >= 4 && msges[3].Trim().ToUpper() == "TX")
                {
                    ...
                    Task.Run(async () =>
                    {
                        ...
                        Thread.Sleep(hsConfig.TxOnlineToResolution);
